...
export function makeBots(): User[] {
  ...
  for (let i = 0; i < 60; i++) {
    ...
    bots.push({
      ...
      stats: { sent: Math.floor(r() * 60), received: Math.floor(r() * 45), caught: Math.floor(r() * 40), distanceKm: Math.floor(r() * 160_000), likes: Math.floor(r() * 300) },
      stamps: [],
      coins: 0,
      inventory: { shield: 0, ufo: 0, orbit: 0, peek: 0, pull: 0, direct: 0, carpet: 0 },
      quota: { date: '', peeks: 0, pulls: 0, earned: 0, month: '', direct: 0 },
      plan,
      shieldMilestone: 0,
