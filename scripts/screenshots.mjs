...
const signup = async (nick = 'dan') => { await tap('Google로 계속'); await page.getByPlaceholder('닉네임 (친구에게만 보여요)').fill(nick); await page.getByPlaceholder('한 줄 소개').fill('밤에 더 살아있는 사람'); await tap('디자인', { exact: true }); await tap('러닝', { exact: true }); await shot('signup-profile'); await tap('시작하기', { exact: true }); await page.waitForTimeout(800); };
